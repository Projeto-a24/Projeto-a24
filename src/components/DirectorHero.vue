...
const currentPair = computed(() => {
  if (!props.featuredDirectors.length) return [];
  const first = props.featuredDirectors[props.currentIndex];
  const second = props.featuredDirectors[(props.currentIndex + 1) % props.featuredDirectors.length];
  return [first, second];
});

const totalPairs = computed(() => Math.ceil(props.featuredDirectors.length / 2));
</script>

<template>
  <section class="hero" v-if="featuredDirectors.length > 0 && currentPair.length">
